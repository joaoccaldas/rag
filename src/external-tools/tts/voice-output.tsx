...
export function VoiceOutput({ 
  ...
  useEffect(() => {
    ...
    window.addEventListener('speak-text', handleSpeakText as EventListener)
    
    return () => {
      window.removeEventListener('speak-text', handleSpeakText as EventListener)
    }
  }, [])
  ...
  useEffect(() => {
    if (autoPlay && isEnabled && currentText && isSupported) {
      speak(currentText)
    }
  }, [currentText, autoPlay, isEnabled, isSupported])

  const speak = (textToSpeak: string = currentText) => {
    if (!isSupported || !textToSpeak || !isEnabled) return
  ...
  return (
    <div className={`flex items-center space-x-1 ${className}`}>
      <button
        ...
        disabled={!isEnabled || !currentText}
        className={`
          ${!isEnabled || !currentText ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
