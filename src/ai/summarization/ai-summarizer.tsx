...
interface AISummarizerProps {
  ...
  className?: string
  autoAnalyze?: boolean // New prop for automatic analysis during document processing
}
...
export function AISummarizer({ 
  ...
  React.useEffect(() => {
    if (autoAnalyze && content && !summaryData && !isLoading) {
      generateSummary()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoAnalyze, content, summaryData, isLoading])
...
Focus on extracting actionable metadata that would be useful for knowledge graph construction and document correlation. Be precise and specific.`
  ...
  // For auto-analyze mode, don't render UI
  if (autoAnalyze && !isExpanded) {
    return (
      <div className={`text-xs text-gray-500 ${className}`}>
        ...
        {error && (
          <div className="text-red-500">AI analysis failed</div>
        )}
      </div>
    )
  }
